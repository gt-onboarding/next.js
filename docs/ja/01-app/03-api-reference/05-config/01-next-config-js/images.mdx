...
{/* このドキュメントの内容は App Router と Pages Router で共通です。Pages Router 固有のコンテンツを追加するには、`<PagesOnly>Content</PagesOnly>` コンポーネントを使用できます。共通のコンテンツはコンポーネントでラップしないでください。 */}

Next.js の組み込みの Image Optimization API の代わりにクラウドプロバイダーを使って画像を最適化したい場合は、`next.config.js` を次のように設定します。
...
この `loaderFile` は、Next.js アプリケーションのルートからの相対パスでファイルを指している必要があります。このファイルは、文字列を返すデフォルト関数を export する必要があります。例えば次のようになります:
...
あるいは、[`loader` prop](/docs/app/api-reference/components/image#loader) を使用して、`next/image` の各インスタンスに関数を渡すこともできます。

> **補足**: 関数を受け取る画像ローダーファイルをカスタマイズする場合、渡された関数をシリアライズするために [Client Components](/docs/app/getting-started/server-and-client-components) を使用する必要があります。

組み込みの [Image Optimization API](/docs/app/api-reference/components/image) と [Image Component](/docs/app/api-reference/components/image) の挙動の設定方法については、利用可能なオプションを [Image Configuration Options](/docs/app/api-reference/components/image#configuration-options) で確認してください。
</AppOnly>
...
あるいは、[`loader` prop](/docs/pages/api-reference/components/image#loader) を使用して、`next/image` の各インスタンスに関数を渡すこともできます。

組み込みの [Image Optimization API](/docs/pages/api-reference/components/image) と [Image Component](/docs/pages/api-reference/components/image) の挙動の設定方法については、利用可能なオプションを [Image Configuration Options](/docs/pages/api-reference/components/image#configuration-options) で確認してください。
